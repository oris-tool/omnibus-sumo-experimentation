...
public class Table2PCSumoAnalyzer {
    ...
    public Table2PCSumoAnalyzer analyze(int flowIndex,
        ...
        for (int r = 0; r < Table2PatternComparator.sumo_runs; r++) {
            ...
            final BigDecimal newRate =
                    BigDecimal.ONE.divide(
                            BigDecimal.ONE.divide(
                                    ...
                                    .subtract(Table2PatternComparator.sumo_shift),
                            5, RoundingMode.HALF_UP);
            ShiftedExpSampleGenerator shiftedExponentialSampler =
                    new ShiftedExpSampleGenerator(newRate, Table2PatternComparator.sumo_shift);
            ...
            // Start simulation
            SumoTraciConnection conn = new SumoTraciConnection(Table2PatternComparator.WIN_SUMO_BIN,
                    Table2PatternComparator.CONFIG_FILE);
            conn.addOption("step-length", Table2PatternComparator.sumo_time_step.toString());
            ...
            for (int ts = 0; ts < SIMULATION_STEPS; ts++) {
                ...
                if ("rGr".equals(tlPhase)) {
                    this.availability[ts] += (1. / (double) Table2PatternComparator.sumo_runs);
                }
...
//                }
                ...
                this.onQueueCars[ts] += carCounters.doubleValue() / (double) Table2PatternComparator.sumo_runs;
            ...
            try {
                Runtime.getRuntime().exec("taskkill /F /IM " + Table2PatternComparator.SUMO_EXE);
            } catch (IOException e) {
                e.printStackTrace();
            }
            try {
